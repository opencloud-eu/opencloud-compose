...
OpenCloud Compose supports adding SSL certificates for public domains and development environments. This feature enables you to use your own certificates instead of relying solely on Let's Encrypt or self-signed certificates. It can also use the Let's Encrypt ACME challenge to generate certificates for your public domains.

### Use Let's Encrypt with ACME Challenge

1. **Enable Let's Encrypt**:
   - Set `TRAEFIK_LETSENCRYPT_ENABLE=true` in your `.env` file
   - Set `TRAEFIK_LETSENCRYPT_EMAIL` to your email address for the ACME challenge
   - Update `TRAEFIK_SERVICES_TLS_CONFIG="tls.certresolver=letsencrypt"` to use Let's Encrypt
...
3. **Configure environment variables**:
   - Set `TRAEFIK_USE_LOCAL_CERTIFICATES=true` in your `.env` file
   - Set `TRAEFIK_LETSENCRYPT_ENABLE=false` to disable Let's Encrypt (if previously enabled)
   - Update `TRAEFIK_SERVICES_TLS_CONFIG="tls=true"` to use your local certificates
   
   ```bash
   # In your .env file
   TRAEFIK_USE_LOCAL_CERTIFICATES=true
...
| `TRAEFIK_LETSENCRYPT_EMAIL`        | Email Address for the Let's Encrypt ACME challenge | (commented out)           |
| `TRAEFIK_LETSENCRYPT_ENABLE`       | Enable Let's Encrypt  ACME                         | false                     |
| `TRAEFIK_USE_LOCAL_CERTIFICATES`   | Use local certificates instead of Let's Encrypt    | false                     |
| `TRAEFIK_SERVICES_TLS_CONFIG`      | Tell the services which TLS config they use        | (commented out)           |
